...
crispritz.py add-variants hg19_1000genomeproject_vcf/ hg19_ref/ > output.redirect.out
echo -e "ADD-VARIANTS \e[32mOK\e[0m"
echo "-------------------------------------------------------------------------"

echo "TESTING INDEX-GENOME"
crispritz.py index-genome hg19_ref hg19_ref/ pam/pamNGG.txt -bMax 2 > output.redirect.out
echo -e "INDEX-GENOME \e[32mOK\e[0m"
echo "-------------------------------------------------------------------------"

echo "TESTING SEARCH WITH ONLY MISMATCHES"
crispritz.py search hg19_ref/ pam/pamNGG.txt guides/EMX1.txt emx1.hg19 -mm 4 -t -scores hg19_ref/ > output.redirect.out
echo -e "SEARCH WITH ONLY MISMATCHES \e[32mOK\e[0m"
echo "-------------------------------------------------------------------------"

echo "TESTING SEARCH WITH MISMATCHES AND BULGES"
crispritz.py search genome_library/NGG_2_hg19_ref/ pam/pamNGG.txt guides/EMX1.txt emx1.hg19.bulges -index -mm 4 -bDNA 1 -bRNA 1 -t > output.redirect.out
echo -e "SEARCH WITH MISMATCHES AND BULGES \e[32mOK\e[0m"
echo "-------------------------------------------------------------------------"

echo "TESTING ANNOTATE-RESULTS"
crispritz.py annotate-results guides/EMX1.txt emx1.hg19.targets.txt annotations_path.txt emx1.hg19.annotated > output.redirect.out
