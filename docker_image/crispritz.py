...
def indexGenome():
	...
	for f in listChrs:
		print("Indexing:", f)
		subprocess.run([origin_path+"/buildTST", str(dirGenome)+"/"+str(f), str(dirPAM), str(variant)])
...
def genomeEnrichment():
	...
	start_time = time.time()
	subprocess.run([origin_path+'/Python_Scripts/Enrichment/bcf_query.sh', dirVCFFiles+'/'])
	...
	for f in listChrs:
		...
		subprocess.run([origin_path+'/Python_Scripts/Enrichment/enricher.py', altfile, genfile])
